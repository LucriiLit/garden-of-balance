  ...
  constructor() {
    ...
    this.translations = {
      ...
      en: {
        ...
        subtitle:
          "National Taipei University of Technology - Interaction Design and Programming in a Smart Space",
...
class FloorMatSimulator {
  ...
  getRainbowColor(groupIndex, totalGroups) {
    ...
    const hue =
      totalGroups <= 1
        ...
        : (groupIndex / Math.max(1, totalGroups - 1)) *
          this.COLOR_MAP.HUE_RANGE;
  ...
  bindEvents() {
    ...
    // 地墊拖曳 / Mat Dragging
    document
      .querySelector(".mat-item")
      .addEventListener("dragstart", this.handleDragStart.bind(this));
    document
      .querySelector(".mat-item")
      .addEventListener("dragend", this.handleDragEnd.bind(this));

    // 清除記錄按鈕 / Clear Log Button
    document
      .getElementById("clear-log")
      .addEventListener("click", this.clearOutputLog.bind(this));
  ...
  handleDrop(e) {
    ...
    if (matType === "existing_mat") {
      ...
      const sourceCell = document.querySelector(
        `[data-row="${sourceRow}"][data-col="${sourceCol}"]`
      );
      ...
      if (cell.classList.contains("occupied")) {
        this.showMessage(
          this.langManager.getText("position-occupied"),
          "warning"
        );
        return;
      }
  ...
  updateMatNumbers() {
    ...
    this.groups.forEach((group, groupIndex) => {
      if (group.size > 1) {
        // 只有多於一個地墊的群組才顯示群組效果 / Only show group effects for groups with more than one mat
        // 獲取群組在多群組中的索引 / Get group index in multi-groups
        const multiGroupIndex = multiGroups.findIndex((g) => g === group);
        const groupColor = this.getRainbowColor(
          multiGroupIndex,
          totalMultiGroups
        );
  ...
  highlightSingleMat(cell) {
    ...
    cell.style.setProperty(
      "transition",
      `background-color ${this.ANIMATION_TIMING.FADE_IN}ms ease-in-out`,
      "important"
    );
    ...
    requestAnimationFrame(() => {
      ...
      cell.style.setProperty(
        "background-color",
        this.ANIMATION_COLORS.HIGHLIGHT,
        "important"
      );
    });
    ...
    cell.fadeTimer = setTimeout(() => {
      ...
      cell.style.setProperty(
        "transition",
        `background-color ${this.ANIMATION_TIMING.FADE_OUT}ms ease-in-out`,
        "important"
      );
      cell.style.backgroundColor = this.ANIMATION_COLORS.DEFAULT;
      cell.style.setProperty(
        "background-color",
        this.ANIMATION_COLORS.DEFAULT,
        "important"
      );
  ...
  async addOutputRecord(time, groupId, matNumber) {
    ...
    record.innerHTML = `
            <div style="font-weight: bold; color: ${
              this.UI_COLORS.LOG_TIME
            };">${time}</div>
            <div style="margin-top: ${
              this.LAYOUT.LOG_MARGIN_TOP
            };">${this.langManager.getText(
      "group-label"
    )}: ${groupId} | ${this.langManager.getText(
      "number-label"
    )}: ${matNumber}</div>
        `;
    ...
    if (typeof firebaseManager !== "undefined" && firebaseManager.initialized) {
      try {
        ...
        console.log(
          `Mat press logged: Group ${groupId}, Mat ${matNumber} at ${time}`
        );
      } catch (error) {
  ...
  clearOutputLog() {
    const outputLog = document.getElementById("output-log");
    outputLog.innerHTML = `<p class="no-records">${this.langManager.getText(
      "no-records"
    )}</p>`;
  ...
  updateUI() {
    ...
    if (zoomPercentageElement) {
      zoomPercentageElement.textContent = `${Math.round(
        this.zoomLevel * 100
      )}%`;
    }
    ...
    document.getElementById(
      "grid-size"
    ).textContent = `${this.gridCols}x${this.gridRows}`;
    ...
    document.getElementById("zoom-in").disabled =
      this.zoomLevel >= this.ZOOM_SETTINGS.MAX;
    document.getElementById("zoom-out").disabled =
      this.zoomLevel <= this.ZOOM_SETTINGS.MIN;
  }
  ...
  getMessageColor(type) {
    switch (type) {
      case "success":
        return this.MESSAGE_COLORS.SUCCESS;
      case "warning":
        return this.MESSAGE_COLORS.WARNING;
      case "error":
        return this.MESSAGE_COLORS.ERROR;
      default:
        return this.MESSAGE_COLORS.INFO;
    }
  }
  ...
  importConfiguration(configuration) {
    ...
    configuration.mats.forEach((mat) => {
      const cell = document.querySelector(
        `[data-row="${mat.row}"][data-col="${mat.col}"]`
      );
      if (cell) {
  ...
  loadConfiguration() {
    try {
      ...
      if (!configString) {
        this.showMessage(
          this.langManager.getText("no-saved-config"),
          "warning"
        );
        return;
      }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  simulator.showMessage(
    simulator.langManager.getText("ready-message"),
    "success"
  );
});
